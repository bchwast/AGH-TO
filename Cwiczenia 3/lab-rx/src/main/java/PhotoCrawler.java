...
public class PhotoCrawler {
    ...
    public void downloadPhotosForQuery(String query) throws InterruptedException {
        photoDownloader.searchForPhotos(query)
                .subscribe(photoSerializer::savePhoto, e -> log.log(Level.SEVERE, "Downloading photo examples error", e));
    }

    public void downloadPhotosForMultipleQueries(List<String> queries) {
        photoDownloader.searchForPhotos(queries)
                .blockingSubscribe(photoSerializer::savePhoto, e -> log.log(Level.SEVERE, "Downloading photo examples error", e));
    }
}
